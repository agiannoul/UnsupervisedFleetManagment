...
def resultsCost(filename):
    results=[]
    with open(f'{filename}_KRRESULTS_COST.txt') as file:
        lines = file.readlines()
        for line in lines:
            #print(line)
            PhRange=[i* 5 for i in range(1,14)]
...
def maxresultsCost(filename,PH=30,COST_FN=100):
    ...
    if filename=="KR_TEST/vehicles":
        COSTS=[4,5,10,20]
    phpos=Phrange.index(PH)
    costpos=COSTS.index(COST_FN)
    for re in results:
        cost=re[6][phpos][costpos]
        if mincost>cost:
            mincost=cost
            minExp=c
        c+=1
    return mincost,results[minExp],6 
