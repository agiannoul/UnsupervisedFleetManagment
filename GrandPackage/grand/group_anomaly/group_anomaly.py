...
class GroupAnomaly:
    ...
    def __init__(self, nb_units, ids_target_units, w_ref_group="7days", dynamic_threshold=False, dynamic_error_presentage=0.005, historic_errors=60, w_martingale=15, non_conformity="median", k=20,
                 dev_threshold=.6, transformer=None, w_transform=30, columns=None,R=1.5,metacheck=False,dynTinTout=False):
        ...
        self.detectors = [ IndividualAnomalyInductive(dynamic_threshold=dynamic_threshold, dynamic_error_presentage=dynamic_error_presentage, historic_errors=historic_errors,w_martingale=self.w_martingale, non_conformity=self.non_conformity, k=self.k, dev_threshold=self.dev_threshold,R=R,metacheck=metacheck,dynTinTout=dynTinTout) for _ in range(nb_units) ]
    ...
    def predict(self, dt, x_units):
        ...
        for uid in self.ids_target_units:
            ...
            try:
                ...
                detector.fit(Xref)
                devContext , thhh= detector.predict(dt, x)
            ...
            if thhh==-1 and self.dynamic_threshold:
                thhh=self.Ths[len(self.Ths)-1]
            self.Ths.append(thhh)
        return deviations
    ...
    def plot_deviations(self, figsize=None, savefig=None, plots=["data", "transformed_data", "strangeness", "deviation", "threshold"], debug=False):
        ...
        if any(s in ["pvalue", "deviation", "threshold"] for s in plots):
            fig, axis = plt.subplots(len(self.ids_target_units))
            axs[i].set_xlabel("Time")
            ...
            for uid in self.ids_target_units:
                ...
                axs[i].set_ylabel(uid)
                axs[i].set_ylim(0, 1)
                axis[c].set_ylabel(uid)
                axis[c].set_ylim(0, 1)
                T, P, M ,thresh,deviatingvalues,deviatingTimes= self.detectors[uid].T, self.detectors[uid].P, self.detectors[uid].M ,self.detectors[uid].thresholds,self.detectors[uid].deviatingvalues,self.detectors[uid].deviatingTimes
                if "pvalue" in plots:
                    axis[c].scatter(T, P, alpha=0.25, marker=".", color="green")
                ...
                if "threshold" in plots:
                    if self.dynamic_threshold:
                        axis[c].plot(T, thresh, color='r', label="Threshold")
                    else:
                        axis[c].axhline(y=self.dev_threshold, color='r', linestyle='--', label="Threshold")
                c+=1
        if savefig is None:
            plt.show()
            print("a")
    ...
    def get_information(self,uid):
    	return self.detectors[uid].T, self.detectors[uid].P, self.detectors[uid].M ,self.detectors[uid].thresholds,self.detectors[uid].deviatingvalues,self.detectors[uid].deviatingTimes,self.detectors[uid].NumberoOfN,self.detectors[uid].NumberOfRef
    # ===========================================
