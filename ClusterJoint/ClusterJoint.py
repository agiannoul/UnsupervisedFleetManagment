...
# -*- coding: utf-8 -*-
"""
Created on Thu Mar  3 17:16:02 2022
...
def outlier_removal(X,thstop=-1,count=0):
    ...
    mjoint=np.mean(X, axis=0)
    if count >= 5:
        return X,mjoint 
    distances=[]
    for x in X:
        dist = abs(np.linalg.norm(mjoint-x))
        distances.append(dist)
    mD=statistics.mean(distances)
    if len(distances)==1:
        mStd=0
    else:
        mStd=statistics.stdev(distances)
    if thstop==-1:
        thstop=mD + 0.5*mStd
    if max(distances)<thstop:
        return X,mjoint
    newX=[]
    for tup in zip(X,distances):
        thoutlier=mD + 2*mStd
        if tup[1]<=thoutlier:
            newX.append(tup[0])
    #no outliers
...
def detection(window,shiftdays,dataframes_list,factor):
    ...
    #start=
    TimeIndex=[ [] for i in range(len(dataframes_list))]
    distances_ID=[[] for i in range(len(dataframes_list))]
    for w in range(numberOfwindows):
        ...
        for i in range(0,len(dataframes_list)):
            X=dataframes_list[i][start:end].to_numpy()
            if len(X)==0:
                windowdf_list.append([])
                Clusters.append([])
                continue
            remaining_data,meanofcluster=outlier_removal(X)
            windowdf_list.append(remaining_data)
            Clusters.append(meanofcluster)
        
        
        Cluster_distance=[]
        clusterid=[]
        c=0
        for cmean in Clusters:
            if len(cmean)==0:
                c+=1
                continue
            summ=[]
            for cm in Clusters:
                if len(cm)==0 or (cm==cmean).all():
                    continue
                dist = abs(np.linalg.norm(cmean-cm))
                summ.append(dist)
            Cluster_distance.append(statistics.mean(summ))
            clusterid.append(c)
            #distances_ID[c].append(max(summ))
            #TimeIndex[c].append(end)
            c+=1
        
        
        
        #2T outlier detection
        mesoCluster=statistics.mean(Cluster_distance)
        std=statistics.stdev(Cluster_distance)
        newCluster=[]
        th=mesoCluster+factor*std
        for center_dist in Cluster_distance:
            if center_dist<th:
                newCluster.append(center_dist)
        if len(newCluster)>0:
            mesoCluster=statistics.mean(newCluster)
            if len(newCluster)==1:
                std=0
            else:
                std=statistics.stdev(newCluster)
                th2T=mesoCluster+factor*std
            for c in range(len(Cluster_distance)):
                if Cluster_distance[c]>=th2T:
                    TimeIndex[clusterid[c]].append(dataframes_list[clusterid[c]][start:end].index.max())
...
def plotOutliers(TimeIndex,dataframes_list,indexesforgrand):
    c=0
    for uid in indexesforgrand:
        if True:
            tempdf=dataframes_list[uid]
...
def runsenarioCostBus(filename,w,step,SDfactor):
    ...
    distances,TimeIndex=detectionBus(w,step,dataframes_list,SDfactor)

    #plotResultsBus(TimeIndex,dataframes_list,indexesforgrand)

    phcost=calculateCostBus(TimeIndex,dataframes_list,indexesforgrand)
    #print(Cost)
    towrite=[filename,step,w,SDfactor,phcost]
    with open(f'{filename}_clusterjoint_COST.txt', 'a') as f:
        for item in towrite:
            f.write("%s | " % item)
        f.write("\n" % item)
...
def runsenarioCost(filename,w,step,SDfactor):
    ...
    dflist=loaddflist(filename=filename)
    distances,TimeIndex=detection(w,step,dflist,SDfactor)
    # old plotResults(distances,TimeIndex,dflist,indexesforgrand)
    #plotOutliers(TimeIndex,dflist,indexesforgrand)

    phcost=calculateCost(TimeIndex,dflist,indexesforgrand)
